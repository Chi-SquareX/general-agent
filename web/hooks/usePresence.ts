...
export function usePresence({ socket, sessionId, currentUser }: UsePresenceOptions) {
  ...
  useEffect(() => {
    ...
    cleanup.push(
      socket.on("presence_sync", (e: PresenceSyncEvent) => {
    ...
    cleanup.push(
      socket.on("user_joined", (e: UserJoinedEvent) => {
        setPresence((s) => presenceAddUser(s, e.user));
      })
    );

    cleanup.push(
      socket.on("user_left", (e: UserLeftEvent) => {
        setPresence((s) => presenceRemoveUser(s, e.user.id));
      })
    );

    cleanup.push(
      socket.on("cursor_update", (e: CursorUpdateEvent) => {
    ...
    cleanup.push(
      socket.on("typing_start", (e: TypingStartEvent) => {
    ...
    cleanup.push(
      socket.on("typing_stop", (e: TypingStopEvent) => {
        if (e.userId === currentUser.id) return;
